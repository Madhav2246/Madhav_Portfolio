...
export default function ResearchPanel({ papers }: { papers: ResearchPaper[] }) {
  ...
  return (
    <div className="w-full h-[calc(100vh-0px)] overflow-y-auto panel-scroll relative" style={{ paddingTop: 64 }}>
      {/* Decorative DNA helix-like lines */}
      ...
      <div className="max-w-5xl mx-auto px-8 py-12">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
          ...
          <span style={{ width: 28, height: 1, background: ACCENT, display: "inline-block" }} />Research
        </motion.div>
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="flex flex-col gap-5 pb-10">
          {visible.map(paper => (
            <motion.div key={paper.id} variants={item}
              ...
              <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                ...
                <span className="font-mono text-[9px] tracking-[0.12em] uppercase px-3 py-1 rounded-full"
                      style={paper.status === "published"
                        ? { background: "rgba(56,189,248,0.1)", border: "1px solid rgba(56,189,248,0.3)", color: ACCENT }
                  ...
                  {paper.status === "in-progress"
                    ? <span className="flex items-center gap-1"><span className="w-[5px] h-[5px] rounded-full inline-block" style={{ background: ACCENT, animation: "pulseDot 1.8s ease-in-out infinite" }} /> In Progress</span>
                    : "✓ Published"}
                </span>
              </div>

              <div className="font-mono text-[10px] tracking-[0.14em] uppercase mb-2" style={{ color: "rgba(255,255,255,0.38)" }}>
                {paper.domain}
              </div>
              <h3 className="font-display font-bold text-[18px] text-white mb-3 leading-[1.3]">{paper.title}</h3>
              <p className="text-[15px] leading-[1.78]" style={{ color: "rgba(255,255,255,0.68)" }}>{paper.abstract}</p>

              {paper.metrics.length > 0 && (
                <div className="flex flex-wrap gap-6 mt-6 pt-6" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                  {paper.metrics.map(m => (
                    <div key={m.label}>
                      <div className="font-display font-black text-[24px] text-white leading-none">
                        <CountUp target={m.value} />
                      </div>
                      <div className="font-mono text-[8px] tracking-[0.12em] uppercase mt-1" style={{ color: "rgba(255,255,255,0.32)" }}>
                        {m.label}
                      </div>
                    </div>
                  ))}
